...
{{- end }}
var (
    {{ .Name }}Fields = ` {{ range $i, $f := .Fields }}{{if gt $i 0}}, {{end}}{{ $f.Col.ColumnName }}{{- end }} `
    {{ .Name}}Columns = []string{ {{ range $i, $f := .Fields }}{{if gt $i 0}}, {{end}}"{{ $f.Col.ColumnName }}"{{- end }} }
)
type {{ .Name }} struct {
{{- range .Fields }}
    {{ .Name }} {{ retype .Type }} `json:"{{ .Col.ColumnName }},omitempty"` // {{ .Col.ColumnName }}
{{- end }}
{{- if .PrimaryKey }}
...
func ({{ $short }} *{{ .Name }}) TestOnly_SetExists() {
    {{ $short }}._exists = true
}

// Only for tests usage!
func ({{ $short }} *{{ .Name }}) TestOnly_SetDeleted() {
    {{ $short }}._deleted = true
}
